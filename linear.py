...
if __name__ == '__main__':
    ...
    parser.add_argument('--decl', action='store_true', default=False,
                        help='use decl model weights, save results to the dir results_decl/')
    ...
    model = Net(num_class=len(train_data.classes), pretrained_path=model_path).cuda(2)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.decl:
            data_frame.to_csv('results_decl/linear_statistics.csv', index_label='epoch')
        else:
        ...
        if test_acc_1 > best_acc:
            best_acc = test_acc_1
            if args.decl:
                torch.save(model.state_dict(), 'results_decl/linear_model.pth')
            else:
